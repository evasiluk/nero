...
//modified captcha here;
// it's marked with //modified

if (CModule::IncludeModule("form")) {
    //modified
    \Bitrix\Main\Page\Asset::getInstance()->addString('<script src=\'https://www.google.com/recaptcha/api.js\'></script>');
    $arResult['recaptcha']['div'] = '<div class="g-recaptcha" data-sitekey="' . GRECAPTCHA_PUBLIC . '"></div>';
    $arResult['recaptcha']['error'] = '<span class="error-note" id="recaptchaError"></span>';
    //modified end
    ...
    if (strlen($arResult["ERROR"]) <= 0) {
        ...
        if (($_POST['WEB_FORM_ID'] == $arParams['WEB_FORM_ID'] || $_POST['WEB_FORM_ID'] == $arResult['arForm']['SID']) && (strlen($_REQUEST["web_form_submit"]) > 0 || strlen($_REQUEST["web_form_apply"]) > 0)) {
            ...
            //modified
            if ($arResult["arForm"]["USE_CAPTCHA"] == "Y" || true) {

                
                $recaptcha = new \ReCaptcha\ReCaptcha(GRECAPTCHA_PRIVATE);
                $resp = $recaptcha->verify($_REQUEST['g-recaptcha-response'], $_SERVER['REMOTE_ADDR']);
                if (!$resp->isSuccess()) {
                    $arResult["FORM_ERRORS"][0] = implode(', ', $resp->getErrorCodes());
                }
            }
            //modified end
